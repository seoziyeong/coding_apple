...
app.put("/api/users/update", (req, res) => {
    ...
    // 구조분해를 통해 id 와 name을 추출
    const { id, name } = req.body
    const old_name = users.filter(filtered => filtered.id == id)[0].name;
    ...
    res.json({ok: true, message: `ID ${id}에 해당하는 name 값이 ${old_name} 에서 ${name}으로 변경되었습니다.`})
})
...
app.patch("/api/user/update/:user_id", (req, res) => {
    ...
    const { name } = req.body
    const old_name = users.filter(filtered => filtered.id == user_id)[0].name;
    ...
    res.json({ok: true, message: `ID ${user_id}에 해당하는 name 값이 ${old_name} 에서 ${name}으로 변경되었습니다.`})
})
...
app.delete("/api/user/delete", (req, res) => {
    ...
    //filter라는 함수는 자바스크립트에서 배열 함수이다. 필터링을 할때 많이 사용된다 필터링한 데이터를 새로운 배열로 반환한다.
    users = users.filter(data => data.id != user_id );

    res.json({ok: true, users: users})
})

// http listen port 생성 서버 실행
app.listen(4200, () => console.log("개발이 취미인 남자 :)"));
